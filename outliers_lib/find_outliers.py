...
def find_outliers_quantile(data, feature, left=0.01, right=0.99):
    """
    Находит выбросы в данных, наблюдения которые не попадают в интервал
    обозначенный квантилями, считаются выбросами. Функция будет 
    отсикать 1 процент наблюдений слева и 1 процент справа.
    Args:
        data (pandas.DataFrame): набор данных
        feature (str): имя признака, на основе которого происходит поиск выбросов 
        left (float, optional): граница с левой стороны распределения. По умолчанию 0.01.
        right (float, optional): граница с правой стороны распределения. Defaults to 0.99.
